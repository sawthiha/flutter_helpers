...
class CustomDioForNative with DioMixin implements Dio {

  final FileMode downloadFileMode;

  /// Create Dio instance with default [BaseOptions].
  /// It is recommended that an application use only the same DIO singleton.
  CustomDioForNative({BaseOptions? baseOptions, this.downloadFileMode = FileMode.write}) {
  ...
  Future<dio.Response> download(
    ...
    dio.Response<ResponseBody> response;
    try {
    ...
    RandomAccessFile raf = file.openSync(mode: downloadFileMode);

    // Create a Completer to notify the success/error state.
    final completer = Completer<dio.Response>();
    Future<dio.Response> future = completer.future;
    int received = 0;
